package yourminis.components
{
	...
	public class ThumbCell extends Sprite
	{
		...
		public function ThumbCell( cc:*, labelText:Array, data:*, w:Number, h:Number, tw:Number = 0, cm:Number = 0, overc:Number = 0, overa:Number = 0.5, outc:Number = 0, outa:Number = 0)
		{
			//super( data, w, h );
			cwidth = w;
			twidth = 0;
			...
			if (cc is String) {
				isLoader = true;
				cellContent = new ImageLoader(cc,tw,h,true,false,true);
			}
			else {
				isLoader = false;
				cellContent = cc;
		...
		*/
		...
		private function overHandler( evt:MouseEvent )
		{
			...
			dispatchEvent( new YMListCellEvent(YMListCellEvent.ROLL_OVER, cellIndex, cellData) );
		...
		{
			drawBackground(outColor,outAlpha);
			dispatchEvent(new YMListCellEvent(YMListCellEvent.ROLL_OUT, cellIndex, cellData) );
		}
		...
		*/
		...
		{
			var cellWidth = w;
			var cellHeight = theight + cellPadding*2 + cellMargin;
