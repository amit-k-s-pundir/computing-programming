...
(defparameter *base-dir* (merge-pathnames (make-pathname :directory
                                                         (concatenate 
                                                                     'list
                                                                     (pathname-directory
...
;;Work area;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(setq work-file (ensure-directories-exist (merge-pathnames (make-pathname
                                                            :directory
                                                            (concatenate 'list
                                                                         (pathname-directory
                                                                          *base-dir*)
...
(setq *current-dir* (merge-pathnames (make-pathname :directory (concatenate
                                                                'list
                                                                (pathname-directory
                                                                 *base-dir*)
                                                                '("test")))
...
(with-open-file (s work-file :direction :output :element-type :default
                ...
                (cl-who:with-html-output (s) (loop for (link . title) in
                                                   '(("http://zappa.com/"
                                                      . "Frank Zappa") 
                                ("http://marcusmiller.com/" . "Marcus Miller") 
                                ("http://www.milesdavis.com/" . "Miles Davis")) 
        do (cl-who:htm (:a :href link (:b (cl-who:str title))) :br))))
...
;;;;;;;;;;;;;fad utilities;;;;;;;;;;;;;;;;;;;;;;
(defun mkdir (dir-tree)
  "If the dir-tree is not in the canonical dir-tree representation *and* if it
is just a symbol then a file is created with that symbol-name."
